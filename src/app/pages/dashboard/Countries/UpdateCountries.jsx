...
export const UpdateCountries = () => {
  ...
  const handleEdit_Add_submit = async (event) => {
    ...
    try {
      await axiosInstance.post(`/admin-countries/${id}`, formData).then(response => {
        Swal.fire({
          title: "Success",
          icon: "success",
  ...
  const get_data = async () => {
    await axiosInstance.get(`/admin-countries/${id}`).then(function (res) {
    ...
    <motion.nav
      ...
      <div className=" mb-5 mt-5 d-flex flex-stack">
        <div className="page-title d-flex flex-column justify-content-center flex-wrap me-3">
          <h1 className="page-heading d-flex text-dark fw-bold fs-3 flex-column justify-content-center my-0">{intl.formatMessage({ id: 'Country.Edit' })}</h1>
          <ul className="breadcrumb breadcrumb-separatorless fw-semibold fs-7 my-0 pt-1">
            ...
            <li className="breadcrumb-item text-muted">{intl.formatMessage({ id: 'Menu.Countries' })}</li>
          </ul>
        </div>
        <div>
          <Link to='/Countries' type='button' className='btn btn-primary' >
