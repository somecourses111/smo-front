...
const PostMoments = ({className , row , Delete}) => {
  const intl = useIntl()
  const user = row.user;
  return (
    ...
    <div className='card-body pb-0'>
      ...
      <div className='mb-5'>
        ...
        {/* begin::Toolbar */}
        <div className='d-flex align-items-center mb-5'>
          <Link to={`/Posts/Comments/${row.id}/moment`}  className='btn btn-sm btn-light btn-color-muted btn-active-light-success px-4 py-2 me-4'>
            <KTSVG path='/media/icons/duotune/communication/com012.svg' className='svg-icon-3' />
            {row.comments}
          </Link>

          <Link to={`/Posts/Likes/${row.id}/moment`} className='btn btn-sm btn-light btn-color-muted btn-active-light-danger px-4 py-2' >
